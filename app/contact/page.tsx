import { generateWhatsAppUrl, generateGeneralEnquiryMessage } from '@/lib/whatsapp';

export default function Contact() {
  return (
    <div className="min-h-screen bg-[#0E0E0E]">
      {/* Hero Section */}
      <section className="bg-gradient-to-b from-[#0E0E0E] to-[#1A1A1A] py-16 lg:py-24">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-6 font-heading">
              Contact ADNEX
            </h1>
            <p className="text-xl text-[#B3B3B3] leading-relaxed">
              Get in touch via WhatsApp for immediate assistance with your equipment needs
            </p>
          </div>
        </div>
      </section>

      {/* Main Content */}
      <section className="py-16 lg:py-24">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-4xl mx-auto">
            {/* Primary CTA */}
            <div className="bg-[#2A2A2A] rounded-2xl p-8 md:p-12 border border-[#F2B705]/30 mb-12 text-center">
              <div className="w-20 h-20 bg-[#25D366] rounded-full flex items-center justify-center mx-auto mb-6">
                <svg
                  className="w-10 h-10 text-white"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
              </div>
              <h2 className="text-3xl font-bold text-white mb-4 font-heading">
                WhatsApp Business
              </h2>
              <p className="text-lg text-[#B3B3B3] mb-8 leading-relaxed">
                ADNEX operates primarily through WhatsApp Business for direct, real-time communication. 
                Click below to start a conversation with pre-filled enquiry message.
              </p>
              <a
                href={generateWhatsAppUrl(generateGeneralEnquiryMessage())}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block bg-[#25D366] hover:bg-[#20BA5A] text-white font-semibold py-4 px-8 rounded-lg transition-colors duration-200 text-lg"
              >
                Open WhatsApp Chat
              </a>
            </div>

            {/* Contact Information */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              <div className="bg-[#2A2A2A] rounded-lg p-6 border border-[#2A2A2A]">
                <h3 className="text-xl font-bold text-white mb-3 font-heading flex items-center">
                  <svg className="w-5 h-5 text-[#F2B705] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                  Location
                </h3>
                <p className="text-[#B3B3B3]">
                  Oman & GCC Region
                </p>
              </div>

              <div className="bg-[#2A2A2A] rounded-lg p-6 border border-[#2A2A2A]">
                <h3 className="text-xl font-bold text-white mb-3 font-heading flex items-center">
                  <svg className="w-5 h-5 text-[#F2B705] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  Business Hours
                </h3>
                <p className="text-[#B3B3B3]">
                  Monday - Saturday<br />
                  (Oman Time Zone)
                </p>
              </div>
            </div>

            {/* Enquiry Types */}
            <div className="mb-12">
              <h2 className="text-2xl font-bold text-white mb-6 font-heading">
                What You Can Enquire About
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  'Equipment specifications and availability',
                  'Pricing and payment terms',
                  'Import/export services',
                  'Delivery timelines and logistics',
                  'Equipment condition and verification',
                  'Documentation requirements',
                ].map((item, index) => (
                  <div
                    key={index}
                    className="bg-[#2A2A2A] rounded-lg p-4 border border-[#2A2A2A] flex items-start"
                  >
                    <span className="text-[#F2B705] mr-3 mt-1">•</span>
                    <span className="text-[#B3B3B3]">{item}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* Quick Links */}
            <div className="bg-[#2A2A2A] rounded-lg p-8 border border-[#2A2A2A]">
              <h3 className="text-xl font-bold text-white mb-4 font-heading">
                Quick Links
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <a
                  href="/equipment"
                  className="text-[#F2B705] hover:text-[#E0A904] transition-colors flex items-center"
                >
                  <span className="mr-2">→</span>
                  View Equipment Catalogue
                </a>
                <a
                  href="/services"
                  className="text-[#F2B705] hover:text-[#E0A904] transition-colors flex items-center"
                >
                  <span className="mr-2">→</span>
                  Import & Export Services
                </a>
                <a
                  href="/how-it-works"
                  className="text-[#F2B705] hover:text-[#E0A904] transition-colors flex items-center"
                >
                  <span className="mr-2">→</span>
                  How It Works
                </a>
                <a
                  href="/about"
                  className="text-[#F2B705] hover:text-[#E0A904] transition-colors flex items-center"
                >
                  <span className="mr-2">→</span>
                  About ADNEX
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
